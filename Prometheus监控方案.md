...
```
1. Prometheus负责收集数据，Grafana负责展示数据
```
...
```
3. Alertmanager，负责告警规则 webhook负责钉钉告警
```

##### 架构`下面这张图说明了Prometheus的整体架构，以及生态中的一些组件作用:` 
...
##### 安装chrony：    apt -y  install chrony ntpdate     
...
```
选择 名称、唯一标识符（uid）、默认放在Genearl的夹子里 --->选择数据源 Prometheus   ---> Import（导入）
```
...
```
root@HBD907:~# apt install -y ubuntu-drivers-common ubuntu-drivers 
```

```
root@HBD907:~# ubuntu-drivers devices  ##获取推荐显卡驱动
```
...
```
root@HBD907:~# nvidia-smi -L  ##再次查看当前显卡驱动 看到GPU型号即可成功
```
...
```
root@HBD907:~# apt update &&  apt install -y golang-go
```
...
```
root@HBD907:~# which dcgm-exporter   ##查看dcgm-exporter命令路径
```

```
root@HBD907:~/gpu-monitoring-tools# dcgm-exporter ##启动dcgm
```

```
root@HBD907:~# curl 192.168.6.21:9400/metrics   ##测试看到监控数据 ip地址为客户端ip
```
...
```
systemctl daemon-reload  && systemctl enable dcgm-exporter.service
```

```
systemctl start dcgm-exporter.service  &&  systemctl status dcgm-exporter.service
```
...
```
curl http://127.0.0.1:9400/metrics  ##查看抓取的数据
```
...
```
docker run -d -p 8060:8060 --name webhook timonwong/prometheus-webhook-dingtalk --ding.profile=webhook1=https://oapi.dingtalk.com/robot/send?access_token=4a94aac99a0a215192cfe04fce89a604a85f990739aa469cc0a172b629223046  
```
